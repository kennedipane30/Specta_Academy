...
@section('content')
    ...
    {{-- Notifikasi Info jika ada pesan dari sistem --}}
    @if(session('info'))
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        @foreach($classes as $c)
            @php
                // Cek apakah ID kelas ini ada dalam daftar jadwal hari ini
                $canAbsen = in_array($c->class_modelsID, $jadwalHariIni);
            @endphp

            <!-- CARD KELAS -->
            <div class="bg-white p-6 rounded-3xl shadow-sm border-l-8 transition duration-300 {{ $canAbsen ? 'border-green-500 shadow-md' : 'border-gray-200 opacity-90' }}">
                <div class="flex justify-between items-start">
                    ...
                    @else
                        <span class="bg-gray-100 text-gray-300 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider">
                            No Attendance
                        </span>
                    @endif
                </div>
                ...
                <div class="my-6">
                    ...
                    @else
                        {{-- MODIFIKASI: Pesan disederhanakan sesuai permintaan --}}
                        <div class="bg-gray-50 p-4 rounded-xl border border-dashed border-gray-200 flex items-center justify-center">
                            <p class="text-gray-400 text-sm font-medium italic">
                                Absensi untuk kelas ini tidak ada
                            </p>
                        </div>
                    @endif
                </div>
                ...
                <div class="flex items-center justify-between mt-4">
                    ...
                    @else
                        <div class="flex items-center text-gray-300 font-bold text-xs uppercase tracking-tight">
