...
@section('title', 'Spekta Promo Management')
...
<div class="animate__animated animate__fadeIn">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
            <h1 class="text-3xl font-black text-gray-800 tracking-tight">MANAGE <span class="text-spekta">PROMO</span></h1>
            <p class="text-gray-500 text-sm">Manage discount strategies and mobile app promotion banners.</p>
        </div>
        @if(session('success'))
            <div class="bg-green-500 text-white px-6 py-2 rounded-2xl shadow-lg flex items-center gap-2 animate__animated animate__bounceIn">
                ...
                <span class="font-bold text-xs uppercase">Successfully Published!</span>
            </div>
        @endif
    </div>
    ...
    <div class="bg-white rounded-[2rem] shadow-2xl shadow-red-100 overflow-hidden border border-gray-100 mb-12">
        ...
        <form action="{{ route('admin.promo.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div class="space-y-6">
                    <div class="group">
                        <label class="block text-xs font-black text-gray-400 uppercase mb-2 ml-1">1. Banner Visual</label>
                        ...
                        <div class="relative">
                            <select name="class_id" class="w-full bg-gray-50 border-2 border-gray-100 p-4 rounded-2xl text-sm focus:border-red-500 outline-none appearance-none transition shadow-sm" required>
                                ...
                                @foreach($classes as $c)
                                    {{-- Menggunakan class_id & program_name --}}
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="col-span-2 md:col-span-1">
                            <label class="block text-xs font-black text-gray-400 uppercase mb-2 ml-1">3. Secret Code</label>
                            <input type="text" name="code" placeholder="Ex: SPEKTA77" class="w-full bg-gray-50 border-2 border-gray-100 p-4 rounded-2xl text-sm font-mono font-bold tracking-widest focus:border-red-500 outline-none transition shadow-sm" required>
                        </div>
                        <div class="col-span-2 md:col-span-1">
                            <label class="block text-xs font-black text-gray-400 uppercase mb-2 ml-1">4. Percentage (%)</label>
                            <div class="relative">
                    ...
                    <div>
                        <label class="block text-xs font-black text-gray-400 uppercase mb-2 ml-1">5. Validity Period</label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="bg-white rounded-[2rem] border border-gray-100 shadow-xl hover:shadow-2xl transition-all duration-500 group relative">
            <div class="h-44 overflow-hidden rounded-t-[2rem] relative">
                ...
                <div class="absolute bottom-4 left-6 text-white">
                    {{-- relasi class & program_name --}}
                    <span class="bg-red-600 text-[10px] font-bold px-3 py-1 rounded-full uppercase">{{ $row->class->program_name ?? 'General' }}</span>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <form action="{{ route('admin.promo.destroy', $row->promotion_id) }}" method="POST">
                    @csrf @method('DELETE')
                    <button type="submit" class="w-full bg-gray-50 text-red-500 py-3 rounded-xl text-xs font-black uppercase hover:bg-red-500 hover:text-white transition duration-300 border border-red-50" onclick="return confirm('Delete this promo?')">
                        Delete Campaign
                    </button>
                </form>
            </div>
        ...
        @empty
            <div class="md:col-span-3 text-center py-24 bg-gray-50 rounded-[3rem] border-4 border-dashed border-white">
                <p class="text-gray-400 font-bold uppercase tracking-widest text-sm">No Active Campaigns</p>
            </div>
        @endforelse
    </div>
</div>
