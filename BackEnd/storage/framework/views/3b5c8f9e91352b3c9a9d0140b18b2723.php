...
<div class="pq-page">
    ...
    <section class="pq-manual-grid">
        ...
        <div class="pq-manual-card">
            <div class="pq-panel-head">
                <div>
                    ...
                    <p>Isi detail pertanyaan untuk menyicil soal (Soal Ke-<?php echo e($draftCount + 1); ?>).</p>
                </div>
            </div>

            <form action="<?php echo e(route('pengajar.latihan.store_draft', $class->class_id)); ?>" method="POST">
                <?php echo csrf_field(); ?>
                <input type="hidden" name="subject" value="<?php echo e($subject_name); ?>">
                ...
                <div class="pq-field" style="margin-bottom: 15px;">
                    ...
                    <div class="pq-input-wrap">
                        <i class="fa-solid fa-calendar-week"></i>
                        <?php if($activeDraftWeek): ?>
                            <!-- Jika sudah ada draf, kunci minggu agar tidak tercampur -->
                            <input type="hidden" name="week" value="<?php echo e($activeDraftWeek); ?>">
                            <select disabled style="background: #e5e7eb; cursor: not-allowed;">
                                <option>Terkunci di Minggu Ke-<?php echo e($activeDraftWeek); ?> (Selesaikan draf)</option>
                            </select>
                        <?php else: ?>
                            <select name="week" required>
                                <?php for($i = 1; $i <= 20; $i++): ?>
                                    <option value="<?php echo e($i); ?>">Minggu Ke-<?php echo e($i); ?></option>
                                <?php endfor; ?>
                            </select>
                        <?php endif; ?>
                    </div>
                ...
                <div class="pq-form-actions" style="margin-top: 20px;">
                    <?php if(!$isDraftValid): ?>
                        ...
                        <button type="submit" class="btn-full red">
                            <i class="fa-solid fa-paper-plane"></i> Simpan ke Draf (Soal Ke-<?php echo e($draftCount + 1); ?>)
                        </button>
                    <?php else: ?>
            ...
            <?php if($isDraftValid): ?>
                <form action="<?php echo e(route('pengajar.latihan.publish_draft', $class->class_id)); ?>" method="POST" style="margin-top: 10px;">
                    <?php echo csrf_field(); ?>
                    <input type="hidden" name="subject" value="<?php echo e($subject_name); ?>">
                    <button type="submit" class="btn-full green" onclick="return confirm('Terbitkan <?php echo e($draftCount); ?> soal ini sekarang?')">
                        <i class="fa-solid fa-rocket"></i> TERBITKAN <?php echo e($draftCount); ?> SOAL INI KE SISWA
                    </button>
                </form>
            <?php endif; ?>
        </div>
        ...
        <div class="pq-manual-card">
            <div class="pq-panel-head">
                <div>
                    <h2>Soal Terkirim (<?php echo e($draftCount); ?>)</h2>
                    <p>Draf soal Anda (Minimal 5 soal untuk terbit).</p>
                </div>
                <div class="draft-indicator <?php echo e($isDraftValid ? 'valid' : 'invalid'); ?>">
                    <?php echo e($draftCount); ?>/5
                </div>
            </div>

            <div class="pq-draft-list">
                <!-- ✨ PENGAMANAN LOOPING DRAFT -->
                <?php if(isset($drafts) && is_array($drafts)): ?>
                    <?php $__empty_1 = true; $__currentLoopData = $drafts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $draft): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <?php if(is_array($draft)): ?>
                            <div class="draft-item">
                                <div class="draft-number"><?php echo e($index + 1); ?></div>
                                <div class="draft-content">
                                    <strong><?php echo e(Str::limit($draft['question'] ?? 'Soal', 40)); ?></strong>
                                    <span>Kunci: <?php echo e($draft['correct_answer'] ?? '-'); ?></span>
                                </div>
                                <a href="<?php echo e(route('pengajar.latihan.delete_draft', ['class_id' => $class->class_id, 'draft_id' => $draft['id'] ?? '0', 'subject' => $subject_name])); ?>"
                                   class="draft-delete"
...
<?php echo $__env->make('layouts.spekta', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Windows\Documents\GitHub\Specta_Academy\BackEnd\resources\views/pengajar/Latihan/pilih.blade.php ENDPATH**/ ?>