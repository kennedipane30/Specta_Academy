...
<div class="space-y-8">
    <div class="bg-white p-8 rounded-[2rem] shadow-sm border-t-8 border-[#990000]">
        <h3 class="text-2xl font-black text-gray-800 uppercase tracking-tight mb-2">Publish Announcement</h3>
        <p class="text-xs text-gray-400 font-bold uppercase tracking-widest mb-8">This information will appear directly on the student mobile app</p>

        <form action="{{ route('admin.announcement.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input type="text" name="title" placeholder="Announcement Title" class="p-3.5 rounded-2xl bg-gray-50 border-none shadow-inner w-full font-bold" required>
            ...
            <textarea name="description" rows="3" placeholder="Announcement description..." class="p-4 rounded-2xl bg-gray-50 border-none shadow-inner w-full font-medium" required></textarea>
        ...
        <div class="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-gray-100 flex flex-col">
            ...
            <div class="p-6 flex-1">
                ...
                <div class="flex gap-2 pt-4 border-t border-gray-50">
                    {{-- announcement_id --}}
                    <a href="{{ route('admin.announcement.edit', $row->announcement_id) }}" class="flex-1 text-center bg-blue-50 text-blue-600 py-2 rounded-xl text-[10px] font-black uppercase hover:bg-blue-600 hover:text-white transition">Edit</a>
                    <form action="{{ route('admin.announcement.destroy', $row->announcement_id) }}" method="POST" class="flex-1">
                        @csrf @method('DELETE')
                        <button class="w-full bg-red-50 text-red-600 py-2 rounded-xl text-[10px] font-black uppercase hover:bg-red-600 hover:text-white transition" onclick="return confirm('Delete?')">Delete</button>
                    </form>
                </div>
            </div>
        </div>
